...
\item {\em Finite interpretations} have only finite domains.
      ...
      At each iteration the formulae are reduced to function free clause logic AND I NEED TO
      READ MORE \cite{CS03,McC03-MACE4-TR,BF+09}
...
\item {\em Infinite interpretations} have one or more infinite domains.
      ...
      HOW IS THIS DONE?
      There are several ATP systems that produce infinite models, e.g., 
...
\item Comprehensibility - the extent to which the interpretation can be understood, typically
      ...
      This property suggests that interpretations should be written using the same vocabulary
      as the formulae, and even better using the same syntax.
\end{packed_itemize}

The new TPTP format represents interpretations as formulae, written in an extension of the
language of the formulae being interpreted.
That immediately contributes to comprehensibility (but does not ensure it).

There are two obvious ways to interpret a formula wrt a given interpretation.
\begin{packed_itemize}
\item For a Kripke interpretation with finite worlds~\ldots
      \begin{packed_itemize}
      \item Interpret quantifiers over worlds using Kripke semantics.
      \item Interpret formulae with a world using what follows~\ldots
      \end{packed_itemize}
\item For a Tarskian interpretation with finite domains or no quantification, and mappings for 
      ...
      \begin{packed_itemize}
      \item Interpret quantifiers for terms using Tarskian semantics.
      \item Interpret ground (grounded with domain elements) terms and atoms using the mappings.
      \item Interpret boolan formulae using truth tables for connectives.
      \end{packed_itemize}
\item For an interpretation represented by formulae, theorem proving can be used~\ldots
      \begin{packed_itemize}
      \item If a formula can be proved from an interpretation it is $true$ in the interpretation.
      \item If a formula can be disproved from an interpretation it is $false$ in the 
            interpretation.
      \item In practice a formula can be neither proved nor disproved within limited/reasonable 
            resource limits, so that nothing is known.
      \item In the case that a set of Herbrand interpretations is induced by a set of formulae, 
            GOTTA WORK THIS OUT.
      \end{packed_itemize}
\end{packed_itemize}
...
i.e., $I$ is a model of formula $\Phi$.
      ...
      Finite interpretations are typically verifiable, tractable, and comprehensible.

      Infinite interpretations are verifiable, but often intractable and might not be 
      comprehensible.

            Saturations are verifiable, often intractable, and almost always incomprehensible.
            Formulae as Herbrand interpretations are verifiable, and they can be tractable and
            comprehensible.

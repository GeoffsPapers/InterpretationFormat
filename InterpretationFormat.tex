...
\usepackage{hyperref}
\usepackage{xr}
\externaldocument{InterpretationExamples}
...
\conference{The 10th Workshop on Practical Aspects of Automated Reasoning,
  as part of the Federated Logic Conference (FLoC) 2026, Lisbon, Portugal, July 25, 2026}
...
\address[4]{Ludwig-Maximilians-Universität München, Germany}

\begin{abstract}
This paper describes the (new) TPTP format for representing interpretations.
The sources and properties of interpretations, which influenced the design of the format, are
discusse.
The format for Tarskian, Herbrand, and Kripke interpretations is described.
Tools for verification and visualization of the interpretations are described.
\end{abstract}
\begin{keywords}
  TPTP World \sep
  Interpretations
...
%----EasyChair style
%EasyChair \title{The TPTP Format for Interpretations}
%EasyChair 
%EasyChair \author{
%EasyChair   Geoff Sutcliffe\inst{1}
%EasyChair \and
%EasyChair   Alexander Steen\inst{2}
%EasyChair \and
%EasyChair   Pascal Fontaine\inst{3}
%EasyChair \and
%EasyChair   Lydia Kondylidou\inst{4}
%EasyChair }
%EasyChair 
%EasyChair \institute{
%EasyChair   University of Miami,
%EasyChair   Miami, USA\\
%EasyChair   \email{geoff@cs.miami.edu}
%EasyChair \and
%EasyChair   University of Greifswald,
%EasyChair   Greifswald, Germany\\
%EasyChair   \email{alexander.steen@uni-greifswald.de}
%EasyChair \and
%EasyChair   University of Li{\`e}ge,
%EasyChair   Li{\`e}ge, Belgium\\
%EasyChair   \email{Pascal.Fontaine@uliege.be}
%EasyChair \and
%EasyChair   Ludwig-Maximilians-Universit{\"a}t München
%EasyChair   Munich, Germany\\
%EasyChair   \email{Lydia.Kondylidou@tcs.ifi.lmu.de}
%EasyChair }
%EasyChair 
%EasyChair \authorrunning{Sutcliffe, Steen, Fontaine, Kondylidou}
%EasyChair \titlerunning{TPTP World Interpretations}
%EasyChair 
%EasyChair \begin{document}

\maketitle
%--------------------------------------------------------------------------------------------------
%----EasyChair style
%EasyChair \begin{abstract}
%EasyChair This paper describes the TPTP format for representing interpretations.
%EasyChair It provides a background survey that helped ensure that the representation format is adequate
%EasyChair for different types of interpretations: Tarskian, Herbrand, and Kripke interpretations.
%EasyChair The needs of applications that use models are considered.
%EasyChair The syntax and semantics of the format is expounded in detail, with multiple examples.
%EasyChair Verification of models is discussed.
%EasyChair Some tools that support processing the format are noted.
%EasyChair The properties of interpretations represented in the format are discussed.
%EasyChair \end{abstract}
%--------------------------------------------------------------------------------------------------
...
\paragraph{Note well:} 
This paper is self-contained, but refers to examples of problems and their interpretations in 
TPTP format that are in the appendices of the extended version of this paper~\cite{SS+26-arXiv}.
They are referenced with a \example{} icon.
It is highly recommended that readers of this paper have~\cite{SS+26-arXiv} available.
...
to $\{\textit{true},\textit{false}\}$~\cite{Hun96,Gal15}.
...
\hspace*{0.9cm}\smalltt{( is\_descendant(int2person(A),int2person(D)) <=> peano\_less(A,D) )}\\
...
%--------------------------------------------------------------------------------------------------
\subsection{Visualization of Finite Tarskian Interpretations}
\label{IIV}
...
The finite Tarskian interpretations for the worlds are provided in the format described 
in Section~\ref{NewTarskian}, e.g.,: \\
...
\hspace*{0.9cm}\smalltt{! [W: \$world] : \$in\_world(W,}\emph{the domain specification}\smalltt{).} \\
...
\hspace*{0.9cm}\smalltt{! [W: \$world] : \$in\_world(W, ( charly = d2child(child\_1) )) ).} \\
...
\hspace*{0.9cm}\smalltt{! [W: \$world] : \$in\_world(W, rains) ).} \\
contains the predicate mapping for \smalltt{rains} that is the same in all worlds
[\example{}:~\ref{NXF_Finite-Finite-Global.s}].

%--------------------------------------------------------------------------------------------------
\subsection{Verification of Kripke Models}
...
%--------------------------------------------------------------------------------------------------
\subsection{Visualization of Finite-Finite Kripke Interpretations}
\label{IKV}

Finite-finite Kripke interpretations with domain types, as described in 

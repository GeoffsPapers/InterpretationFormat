...
A {\em Kripke interpretation} \cite{Kri63} adds a layer of so-called {\em possible worlds} over Tarskian 
interpretations.
...
Within each world there is a Tarskian interpretation, each possibly having different domains, and terms
may denote objects that do not exist (in the respective domain) at the world of evaluation.
If {\em local terms} are assumed, the denotation of any ground term at some world may only be
chosen from the domain of that world. Quantification only quantifies over objects of the domain from the
world at hand ({\em actualist quantification}), but the domains of the Tarskian interpretations at each
world may be chosen to coincide in order to simplify matters ({\em possibilist quantification}).
Formulae can be interpreted wrt Kripke interpretations (with finite worlds and local terms)~\ldots
\begin{packed_itemize}
\item Interpret quantifiers over worlds using Kripke semantics (modal operators).
...
Verification has (at least) the following aspects: \alex{I argue that point 3. is validaton and not verification. So verification would be 1,2,4 and validation would be 3. Both is important, of course, but if we split this we can argue that we can fully verify but we cannot validate.}
\begin{packed_enumerate}
\item Are the type declarations and interpretation formula syntactically well-formed 
      and well-typed? \alex{Types are syntax, see e.g. \url{https://lawrencecpaulson.github.io/2022/03/16/Types_vs_Sets.html} :-)}
...
\begin{enumerate}
\item This can be confirmed using standard parsing and type checking tools, e.g., \cite{VS06,HR15,Ste23-TU}.
...
\item The TPTP World uses the theorem proving approach to verification, in which the problem 
      ...
      theorem prover.  \alex{Usually $\Phi$ is used as set, and $\varphi$ as formula. Update here?}
...
FOF interpretation-formulae have a single untyped (or, if you will, of type {\tt \$i}) domain whose \alex{TPTP types never introduced before. Ignore?}

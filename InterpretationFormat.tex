...
Examples of single Tarskian interpretation-formulae are in~\cite[\S\ref{FOF_Finite.s}, 
\S\ref{TFF_Finite.s}, \S\ref{TFF_Peano.s}, \S\ref{TFF_Integer.s}, 
...
even if they make no sense in a typed sense, e.g.,~
in~\cite[\S\ref{FOF_Finite.s}]{SS+26-Appendix}: \\
...
\hspace*{0.5cm}\smalltt{mix = ( \^ [F: syrup > beverage] : ( d2beverage @ d\_coffee ) )}\\
\hspace*{0.5cm}\smalltt{heated\_mix = ( \^ [F: syrup > beverage] : ( d2beverage @ d\_coffee ) )}\\
...
in~\cite[\S\ref{FOF_Finite.s}, \S\ref{TFF_Finite.s}]{SS+26-Appendix} respectively, which separate
the domain specifications from the symbol mappings, 
e.g.,~in~\cite[\S\ref{TFF_Finite.s}]{SS+26-Appendix}:\\
\hspace*{1.0em}\smalltt{tff(garfield\_domains,interpretation-domains} \\
contains the information about the two domains, and the interpretation-formula: \\
\hspace*{1.0em}\smalltt{tff(garfield\_mappings,interpretation-mappings} \\
...
e.g.,~in~\cite[\S\ref{TFF_Finite.s}]{SS+26-Appendix}:\\
\hspace*{1.0em}\smalltt{tff(garfield\_domain\_human,interpretation-domains(human,d\_human)} \\
contains the \smalltt{d\_human} domain specification, and: \\
\hspace*{1.0em}\smalltt{tff(garfield\_domain\_cat,interpretation-domains(cat,d\_cat)} \\
...
\hspace*{1.0em}\smalltt{thf(hot\_coffee\_beverage,interpretation-domains(beverage,d\_beverage)} \\
and: \\
\hspace*{1.0em}\smalltt{thf(hot\_coffee\_syrup,interpretation-domains(syrup,d\_syrup)} \\
while all the symbol mappings are in: \\
\hspace*{1.0em}\smalltt{thf(hot\_coffee,interpretation-mappings}
...
\cite[\S\ref{THF_Finite_Compact.s}]{SS+26-Appendix} shows an example compacting the
interpretation-formula \smalltt{garfield\_mapping\_loves} 
~in~\cite[\S\ref{TFF_Finite.s}]{SS+26-Appendix}.
In \cite[\S\ref{THF_Finite_Compact.s}]{SS+26-Appendix} universal quantification is used to map 
...
\caption{IIV view of Appendix~\ref{TFF_Finite_SeparateDomains.s}}
\label{IIVPicture}
\end{figure}
...
have some tentative examples that show they can be represented in the format.
...
\item A defined constant \smalltt{\$local\_world} of type \smalltt{\$world\,>\,\$o} to specify 
...
interpretation-formula: \\
\hspace*{0.5cm}\smalltt{tff(people\_mappings,interpretation-mappings} \\
contains the mappings that are the same in all worlds.
...
\caption{IIV view of the Kripke model in Appendix~\ref{NXF_Finite-Finite-Global.s}}
\label{IKVPicture}
\end{figure}

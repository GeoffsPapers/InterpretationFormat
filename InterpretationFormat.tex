...
to help pinpoint the bug would be ``frustrating''. And he should know.}
...
properties of model can be considered.
As indicated, a key property is verifiability.
Verification typically means evaluating the problem formulae wrt the model, to ensure all
evaluate to true.
The process of evaluating formulae wrt an explicit model should be tractable, for direct
use and for verification.
Models should also be comprehensible, so that insights can be gained from manual inspection
\cite{EK+10}.

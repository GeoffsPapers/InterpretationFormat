...
defined as ``Some interpretations are models of Ax, and some interpretations are models of C, and
all models of C are conservative extensions of models of Ax (which means that all models of C are 
models of Ax)''.
...
A simple Tarskian interpretation-formula is a conjunction of components (see simple examples in 
Appendices~\ref{FOF_Finite.s}, \ref{TFF_Finite.s}, \ref{TFF_Finite_SeparateDomains.s}, and
\ref{THF_Finite.s})~\ldots
\begin{packed_itemize}
...
are explained in Section~\ref{NewTarskianCompact}.
...
\item Specification of the distinctness of the domain elements.
      ...
      e.g., in Appendix~\ref{FOF_Finite.s} the domain elements are \smalltt{"double quoted"}.
\end{packed_itemize}
...
\item Specification of the distinctness of the domain elements, e.g.,~in 
...
The interpretation-formulae are preceded by the necessary type declarations~\ldots
\begin{packed_itemize}
...
\item Specification of the distinctness of the domain elements, 
      e.g.,~in Appendix~\ref{TFF_Finite.s}~\ldots\\
      \hspace*{0.5cm}\smalltt{\$distinct(d\_garfield,d\_arlene,d\_nermal)}\\
      and in Appendix~\ref{THF_Finite.s}
...
The mappings of higher-order function symbols are lambda abstractions, e.g.,~in 
Appendix~\ref{THF_Finite.s}~\ldots\\
...
Appendices~\ref{TFF_Finite_SeparateDomains.s.p} and~\ref{THF_Finite.s.p} show the verification
...
Appendix~\ref{TFF_Integer.s} shows the model with a \smalltt{\$int} domain.
...
Appendix~\ref{TFF_Finite.s} the interpretation-formula~\ldots \\
\hspace*{1.0em}\smalltt{tff(garfield\_domains,interpretation-domains} \\
contains the information about the two domains, and the interpretation-formula~\ldots \\
...
in Appendix~\ref{FOF_Finite_Medium.s} there are two \smalltt{interpretation-mappings} 
interpretation-formulae, one for functions and one for predicates.
...
e.g., in Appendix~\ref{TFF_Finite.s} the interpretation-formula~\ldots \\
\hspace*{1.0em}\smalltt{tff(garfield\_domain\_human,interpretation-domains(human,d\_human)} \\
contains the \smalltt{d\_human} domain specification, and~\ldots \\
...
Appendix~\ref{THF_Finite.s}, where the domains are specified separately in the 
interpretation-formulae~\ldots \\
...
Appendix~\ref{TFF_Finite_Compact.s} shows an example compacting the
interpretation-formula \smalltt{garfield\_mapping\_loves} in Appendix~\ref{TFF_Finite.s}
In Appendix~\ref{TFF_Finite_Compact.s} universal quantification is used to map \smalltt{loves} to 
\smalltt{d\_garfield} for all \smalltt{d\_cats}~\ldots \\
...
Figure~\ref{IIVPicture} shows the view of the model in Appendix~\ref{TFF_Finite_SeparateDomains.s}.
...
Note the augmentation of the Tarskian interpretations with subformulae such as~\ldots\\
...
Appendix~\ref{NXF_Finite-Finite-Local.p} shows a NXF problem that has both local and global axioms.
It has a countermodel with finite worlds each of which contains a finite Tarskian interpretation, 
which is shown in Appendix~\ref{NXF_Finite-Finite-Local.s}.
...
the worlds of the interpretation, e.g., in Appendix~\ref{NXF_Finite-Finite-Global_Medium.s} the
interpretation-formula~\ldots \\
...
Appendix~\ref{NXF_Finite-Finite-Global.s}, in which the interpretation-formula~\ldots \\
\hspace*{0.5cm}\smalltt{tff(child\_domains,interpretation-domains(child,child\_d)} \\
contains the information about the domain \smalltt{child\_d}, the interpretation-formula~\ldots \\
\hspace*{0.5cm}\smalltt{tff(people\_mappings,interpretation-mappings(charly,child\_d)} \\
contains the mapping for the \smalltt{charly} constant, and the interpretation-formula~\ldots \\
...
their role in the problem, e.g., in Appendix~\ref{NXF_Finite-Finite-Global.p} the axiom~\ldots \\
...
Appendix~\ref{NXF_Finite-Finite-Global.s.p} has the \smalltt{global} subrole~\ldots \\
...
so the NXF verification (negated) conjecture has the \smalltt{local} subrole~\ldots \\
...
% quoted\smalltt{'} versions of them, e.g., in TFF~\ldots \\

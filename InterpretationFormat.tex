...
of the axioms (maps the axioms to \textit{true}) but not a model for the conjecture (maps the 
conjecture to \textit{false}).
...
\item Evaluation of any formulae wrt an interpretation should be tractable.
      This corresponds to the \emph{atom test} and \emph{formula evaluation} postulates suggested
      by~\cite{FL96,CLP04}.
\item Verification of models should be possible (and hopefully tractable) by checking that the 
      problem formulae evaluate to \textit{true} wrt the model.
...
\item For Herbrand interpretations induced by Herbrand-formulae, theorem proving can be used~\ldots
      \begin{packed_itemize}
      \item If a formula can be proved from the Herbrand-formulae then it is \textit{true} in all 
            the Herbrand interpretations induced by the Herbrand-formulae.
      \item If a formula can be disproved from the Herbrand-formulae then it is \textit{false} in 
            all the Herbrand interpretations induced by the Herbrand-formulae.
      \item In practice, a formula might be neither proved nor disproved within reasonable 
            resource limits, so that nothing is known.
      \end{packed_itemize}
...
%--------------------------------------------------------------------------------------------------
% Not needed for CEUR \bibliographystyle{plain}

...
\item {\em Properties of explicit interpretations}
      ...
      \end{packed_itemize}
      As formulae in the language of the input!
      This provides the basis for verification of models, as explained in 
...
%--------------------------------------------------------------------------------------------------
\subsection{Tarskian Interpretations}
\label{TarskianInterpretations}
...
\label{KripkeInterpretations}

{\em Kripke interpretations} \cite{Kri63} add a layer of {\em worlds} over Tarskian
interpretations.
...
Appendices~\ref{FOF_Finite.s}, \ref{TFF_Finite.s}, \ref{TFF_Integer.s}, and~ \ref{TFF_Peano.s},
...
and~\ref{TFF_Finite_Compact.s}, are explained below.)
...
\item Interpretation of the function symbols, as equalities whose left-hand sides are
      formed from symbols applied to domain elements, and whose right-hand sides
      are domain elements.
...
For FOF interpretations, the domain elements can be any terms.
If plain terms are used their distinctness must be specified either with pairwise
inequality or with the {\tt \$distinct} predicate.
A better alternative is to use {\tt "distinct object"} terms, which are known to be distinct
(and often not used in problems, which makes them easy to identify as domain elements) --
Section Appendices~\ref{FOF_Finite.s}~and~\ref{FOF_Finite_Medium.s} for examples.

For TFF and TXF each domain requires a {\em type-promotion} function that converts domain 
elements to terms, to keep the interpretation formula well-typed.
Each domain is a conjunction of (following along in Appendices~\ref{TFF_Finite.s},
\ref{TFF_Integer.s},~and~\ref{TFF_Peano.s} might help here):
\begin{packed_itemize}
\item The domain type, as a formula that makes the type-promotion function a surjection 
...
\item The domain elements (unless implicit from their defined type), 
      If the domain is finite this is a universally quantified disjunction of equalities whose 
      right-hand sides are the terms. 
      If the domain is infinite this is an existentially quantified formula that captures an 
      infinite disjunction of equalities, e.g., for terms representing Peano numbers as the 
      domain elements:\\
...
\end{packed_itemize}
For TFF and TXF the interpretation of symbols uses the type-promotion functions so that the
signatures of the symbols are respected.

...
The syntax and semantics of the format is expounded in detail, with multiple examples.
...
The Tarskian interpretations' domains might \emph{constant}, i.e.,~the same in all the worlds, but 
might also vary.
...
\hspace*{0.5cm}\smalltt{mix = ( \^ [F: beverage > syrup] : ( d2beverage @ d\_coffee ) )}\\
\hspace*{0.5cm}\smalltt{heated\_mix = ( \^ [F: beverage > syrup] : ( d2beverage @ d\_coffee ) )}\\
...
\item The domain for each problem type, as a formula that makes the type-promotion function a 
      ...
      \smalltt{\$int}), this is unecessary.
\item Specification of the domain elements as an existentially quantified formula that captures an
      ...
      If the domain is a defined type such as \smalltt{\$int} this is unecessary,
...
\hspace*{0.8cm}\smalltt{<=> peano\_less(A,D) )}\\
...
That subformulae requires that the domain element \smalltt{child\_1} exists in the world.
...
To be more informative, {\tt fi\_domain} can be replaced by by {\tt interpretation-domains}, and 
...
\item Comprehensibility - be sufficiently (human) comprehensible to be useful in (human)
      applications.
\end{packed_itemize}

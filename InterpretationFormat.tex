...
has led to the development of a new TPTP format for interpretations.
...
The needs of applications that employ model finding vary according to their use of the model.
...
\item Verification (checking that the formulae evaluate to $true$ wrt the model), should be 
      possible.
\item The process of evaluating formulae should be tractable.
...
\item {\em Finite} Tarskian interpretations have only finite domains.
      ...
      An ATP system can decide if there is a model.
...
A set of formulae might not be intended to represent it's models, e.g., the input to an ATP system. 
In the ATP context, a set of formulae can be generated to confirm the existence of Herbrand models.
Such sets can be the result from an ATP system applying model preserving transformations on its 
...
A {\em Kripke interpretation} \cite{Kri63} for non-classical logic adds a layer of {\em worlds} 
over Tarskian interpretations.
...
The new TPTP format represents interpretations in formulae, written {\em interpretation-formulae}
...
To interpret a formula $\varphi$ wrt a given interpretation $I$~\ldots
\begin{packed_itemize}
\item For Kripke interpretations with finite worlds and local terms~\ldots
      \begin{packed_itemize}
      \item Interpret quantifiers over worlds using Kripke semantics.
      \item Interpret formulae within a world according to the points below for intpreting wrt the
            Tarskian interpretation in the world.
      \end{packed_itemize}
\item For all Kripke interpretations, theorem proving can be used.
      RIGHT ALEX - THAT's WHAT AGMMV DOES FOR TFF?
\item For finite Tarskian interpretations~\ldots
      \begin{packed_itemize}
...
\item For Tarskian interpretations represented in interpretation-formulae, theorem proving can 
      be used.
      \begin{packed_itemize}
      \item If a formula can be proved from the interpretation-formulae then it is $true$ in the 
            ...
            first-order logic in \cite{SS+23-LPAR}, by proving that if $\Phi$ can be proved from 
...
\end{packed_itemize}
$I \vdash \Phi$ means that $\Phi$ is $true$ in $I$, i.e., $I$ is a model of formula $\Phi$.

Given ways to interpret a formula wrt a interpretation, models can be verified.
The TPTP World now offers the AGMV model verifier that uses theorem proving to verify models.
It is available in the SystemOnTSTP \cite{Sut07-CSR} web interface 
...
from the interpretation-formulae $\varphi$.
...
Some of the models are Tarskian models, and some are induced Herbrand models.
...
elements (ala \cite[\S5.3.4]{Gal15}), rather than the traditional presentation of interpretation 

...
Verification means evaluating the problem formulae wrt the model, to ensure all evaluate to true.
The process of evaluating formulae wrt an explicit model should be tractable.
Models should also be comprehensible, so that insights can be gained from manual inspection, e.g.,
\cite{EK+10}.
This property suggests that models should be written using the same vocabulary as the problem
formulae, and even better using the same syntax.
...
Need to evaluated formulae wrt interpretation.
Two ways
\begin{packed_itemize}
...
\item For a Tarskian interpretation with finite domains or no quantification, and mappings for 
      ...
      \end{packed_itemize}
\item For any interpretation use ATP
\end{packed_itemize}

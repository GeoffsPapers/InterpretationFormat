...
This process depends on finding a {\em countermodel} for the conjecture, i.e., finding an 
...
and {\em interpretations} \cite{SS+06,Sut08-KEAPPA}.
...
A {\em Tarskian interpretation} \cite{TV56} of formulae logic consists of a non-empty domain of 
unequal elements for each type used in the formulae (just one domain for untyped logic), mappings 
...
A {\em complete} interpretation can be used to evaluate all formulae that can be written in the 
language of the problem formulae, but a {\em partial} interpretation can be used to evaluate the
...
problem formulae (even the input iteself induces its set of Herbrand models), or be generated 
by an ATP system with the explicit intention of representing Herbrand models:
\begin{packed_itemize}
\item {\em Formulae} that are intended to represent Herbrand interpretations are written 
      {\em Herbrand-formulae} to avoid confusion.
...
\item For Herbrand interpretations induced by Herbrand-formulae, theorem proving can be used~\ldots
      ...
      \item In practice, a formula might be neither proved nor disproved within reasonable 
            resource limits, so that nothing is known.
      \end{packed_itemize}
      {\em I believe that this verification technique works, and so do Stephan Schulz and 
...
The problem has all global axioms and a local conjecture.
...
Kripke models for formulae in NXF/NHF are written as interpretation-formulae in TXF/THF, using 
the new defined type and predicates described in Section~\ref{NewKripke}.
The models are verified using the theorem proving approach described in 
Section~\ref{Verification}.\footnote{%
To date we have done this for NXF problem formulae and TXF interpretation-formnulae. 
Extension to NHF might be possible.}
An NXF verification problem is built from NXF problem formulae and the TXF interpretation-formulae.
